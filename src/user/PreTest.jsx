...
export default function PreTest({ supabase = defaultSupabase, navigate, showAlert }) {
  ...
  async function fetchQuestions() {
    setLoading(true)
    try {
      const { data, error } = await supabase.from('soal').select('*').eq('tipe', 'pretest')
  ...
  async function handleFinish() {
    ...
    let score = 0
    questions.forEach(q => {
      if ((answers[q.id] || '') === (q.jawaban_benar || '').toString()) score += 1
    })

    try {
      const existingHasilId = localStorage.getItem('hasil_id')
      if (existingHasilId) {
        const { error } = await supabase.from('hasil_ujian').update({ skor_pretest: score, answers_pre: answers, updated_at: new Date() }).eq('id', Number(existingHasilId))
        if (error) throw error
      } else {
        const { data, error } = await supabase.from('hasil_ujian').insert([{ peserta_id: pesertaId, skor_pretest: score, answers_pre: answers }]).select('id').single()
        if (error) throw error
        localStorage.setItem('hasil_id', data.id)
      }
      navigate('/learning')
    } catch (err) {
      console.error('save pretest', err)
      if (showAlert) showAlert('Gagal Menyimpan', 'Gagal menyimpan hasil Pre-Test. Silakan coba lagi.', 'error')
      else alert('Gagal menyimpan hasil pretest')
    }
  }

  if (loading) return <div style={{ textAlign: 'center', padding: 40, color: 'var(--text-muted)' }}>Loading soal...</div>

  if (questions.length === 0) return (
    <div className="card container" style={{ textAlign: 'center', maxWidth: 640, margin: '0 auto', padding: 32 }}>
      <h3>Pre-Test SADARI</h3>
      <div>Tidak ada soal pre-test yang tersedia saat ini.</div>
    </div>
  )
  ...
  return (
    <div className="card container" style={{ maxWidth: 780, textAlign: 'left', margin: '0 auto', padding: '32px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <h3 style={{ margin: 0, fontSize: '1.4rem' }}>Pre-Test SADARI</h3>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, paddingTop: 16, borderTop: '1px solid rgba(232, 122, 144, 0.15)' }}>
        ...
        {isLast && (
          <button className="btn btn-primary" onClick={() => {
            if (!answers[q.id]) {
              if (showAlert) showAlert('Pilihan Jawaban', 'Silakan pilih jawaban untuk soal ini sebelum menyelesaikan Pre-Test.', 'info')
